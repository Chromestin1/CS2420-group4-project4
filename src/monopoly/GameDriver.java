...
public class GameDriver {
	...
	/*
	 * Square names in board order (indices 0-39). Kept here for output formatting
	 * since Board.boardSquares is private.
	 */
	private static final String[] BOARD_SQUARES = { "Go", "Mediterranean Avenue", "Community Chest 1", "Baltic Avenue",
	...
	/**
	 * Runs a single simulation of n turns using the given jail strategy. Resets the
	 * board before each simulation so runs are independent.
	 *
	 * @param n the number of turns to simulate
	 * @param j the JailStrategy controlling how the player exits jail
	 * @return a Results record with the turn count, strategy name, and per-square
	 *         landing counts
	 */
	...
	/**
	 * Prints a formatted table showing landing count and percentage for each
	 * square.
	 *
	...
	public static void printResults(Results r) {
		...
		for (int i = 0; i < BOARD_SQUARES.length; i++) {
			...
			System.out.printf("%-5d %-30s %10d %11.4f%%%n", i, BOARD_SQUARES[i], r.landingCounts()[i], pct);
		}
	}

	/**
	 * Prints the top 5 most-landed squares from a single simulation result. Used to
	 * quickly compare convergence across runs.
	 *
	...
	public static void printTopSquares(Results r, int runNum) {
		...
		for (int rank = 0; rank < 5; rank++) {
			...
			used[best] = true;
			System.out.printf("  [%d] %s (%.2f%%)", best, BOARD_SQUARES[best],
	...
	/**
	 * Saves one run to a CSV file. Each row is one board square with
	 * landing count + percentage
	 *
	 * @param r      the results from one run
	 * @param runNum what iteration the simulation is on
	 */
	public static void saveToCSV(Results r, int runNum) {
		String filename = "run" + runNum + "_" + r.strategyName() + "_" + r.turns() + ".csv";
		try {
			PrintWriter pw = new PrintWriter(new FileWriter(filename));
			pw.println("Idx,Square,Landings,Percentage");
			for (int i = 0; i < BOARD_SQUARES.length; i++) {
				double pct = r.landingCounts()[i] * 100.0 / r.turns();
				pw.printf("%d,%s,%d,%.4f%n", i, BOARD_SQUARES[i], r.landingCounts()[i], pct);
			}
			pw.close();
		} catch (IOException e) {
	...
	/**
	 * Runs all 80 required simulations: 2 strategies × 4 turn counts × 10
	 * independent runs. Prints the full landing table for the final run of each
	 * (strategy, n) pair, and prints the top-5 summary for all 10 runs to show
	 * convergence.
	 */
	public static void main(String[] args) {
		 ...
		 * and to implement getName(). Teammates must add these before main() will
		 * compile.
		 */
		JailStrategy[] strategies = { new JailStrategy("A"), new JailStrategy("B") };
